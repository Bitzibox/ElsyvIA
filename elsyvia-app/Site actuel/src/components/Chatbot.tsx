...
import { X, Send, User, Bot, Maximize2, Minimize2 } from 'lucide-react';
    ...
    <>
        ...
        {isOpen && (
          <motion.div
            ...
            className={`fixed z-50 flex flex-col bg-white rounded-2xl shadow-2xl ${
              isExpanded
                ? 'inset-5 md:inset-y-10 md:inset-x-1/4'
                : 'bottom-24 right-5 h-3/4 max-h-[600px] w-11/12 max-w-md'
            }`}
          >
      ...
      >
        <AnimatePresence mode="wait">
          ...
          >
            {/* --- MODIFICATION : Taille de l'icône augmentée --- */}
            {isOpen ? <X size={40} /> : <img src={chatbotIcon} alt="Chat" className="w-16 h-16" />}
          </motion.div>
        </AnimatePresence>
      </motion.button>
    </>
